...
class BaseLaplace:
    ...
    def __init__(
        ...
        self._likelihood: LikelihoodModule = likelihood
        ...
        likelihood.check_sigma_noise(sigma_noise)
        ...
        elif backend is not CurvlinopsGGN and backend is not CurvlinopsEF:
            raise ValueError("Only Curvlinops backends are supported.")
    ...
    def log_likelihood(self) -> torch.Tensor:
        ...
        """
        return self._likelihood.log_likelihood_from_loss(
            self.loss,
            self.n_data,
            self.n_outputs,
            self.sigma_noise,
    ...
    def optimize_prior_precision(
        ...
        """
        # Warn users about link approximation choice for classification-like likelihoods.
        if self._likelihood.is_classification_like():
            warnings.warn(
        ...
        if method == TuningMethod.MARGLIK:
            ...
            if loss is None:
                loss = self._likelihood.default_metric(self.n_outputs, self._device)
                if loss is None:
                    loss = (
                        torchmetrics.MeanSquaredError(num_outputs=self.n_outputs).to(
                            self._device
                        )
                        if self._likelihood.is_regression_like()
    ...
    def _glm_forward_call(
        ...
        f_mu, f_var = self._glm_predictive_distribution(
            x, joint=joint and self._likelihood.is_regression_like()
        )
    ...
    def _glm_predictive_samples(
        ...
        return self._likelihood.transform_function_samples(f_samples)
...
class ParametricLaplace(BaseLaplace):
    ...
    def fit(
        ...
        for data in pbar:
            ...
            self._likelihood.validate_output_targets(out, y)
    ...
    def log_marginal_likelihood(
        ...
        if sigma_noise is not None:
            if not self._likelihood.supports_sigma_noise_change:
    ...
    def __call__(
        ...
        **model_kwargs: dict[str, Any],
        ...
        likelihood = self._likelihood.prediction_kind(fitting)
